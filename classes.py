...
Magias = {
    ...
    '3 > Xama' : {
        ...
        'dano_base': [5,7,0],
        'dano' : [5, 7, 0],
        'golpes': ["caja_dada (1)", 'videiras_sangrentas (2)', 'cura(3)','inventario(4)'],
        'armas' : [],
        'mana' : 150,
        'mana_inicial':150,
        'mana_max' : 300,
...
def equip_talismã_i():
    eqp_arma_inicial(player, 'talismã')
...
def escolha_arma():
    ...
    desc_armas_i()
    print('')
    x = int(input('1) ADAGA | 2) ESPADA_CURTA | 3) CAJADO | 4) VARINHA | 5) MACHADO | 6) TALISMÃ |'))
    if x == 1:
        equip_adaga_i()
